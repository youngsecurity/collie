...
describe("the 1.12.0 run: a member is sent to the run's target and nowhere else", () => {
  ...
  test("a higher fork counter continues the same run without waiting an hour", () => {
    const stepped = run({
      state: "done", to: "v1.12.1+ys.1", runId: RUN, startedAt: T0, updatedAt: T0 + 12_000,
    });
    const at = {
      own: "1.12.1+ys.1", self: "minibuch", turn: formatTurn("minibuch", RUN),
      run: stepped, now: T0 + 18_000,
    };
    expect(followGuards(facts({ ...at, leadRelease: "1.12.1+ys.2" }))).toEqual({
      kind: "follow", tag: "v1.12.1+ys.2", runId: RUN,
    });
    expect(followGuards(facts({ ...at, leadRelease: "1.12.1+ys.1" }))).toMatchObject({ reason: "not-higher" });
    expect(followGuards(facts({ ...at, leadRelease: "1.12.1+ys.0" }))).toMatchObject({ reason: "not-higher" });
  });
