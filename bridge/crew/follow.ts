...
export function followGuards(f: FollowFacts): FollowDecision {
  ...
  // `compareRelease`, not `compareSemver`: on this fork a lead on `1.5.1+ys.2` IS higher than a peer on
  // `1.5.1+ys.1`, and the family filter in `cli/update.ts` keeps a peer from ever taking a tag off its train.
  if (compareRelease(lead, own) <= 0) {
...
function legOf(
  ...
  if (version !== null && compareRelease(version, a.target) >= 0) {
 ...
 *  This fork's `+ys.N` counter is part of the version and STAYS: `1.5.1+ys.1.ab12cd3` reads as
 *  `1.5.1+ys.1` (the sha lands after a dot there, `bridge/version.ts`'s `buildStamp`), so a lead states
 *  the release its peers can actually take, and a `+ys.2` lead reads higher than a `+ys.1` peer. */
function bareVersion(value: string): string | null {
  const trimmed = value.trim();
  const named = trimmed.startsWith("v") ? trimmed : `v${trimmed}`;
  const head = named.split("+")[0] ?? named;
  const fork = forkCounterOf(named);
  const kept = fork === null ? head : `${head}+ys.${fork}`;
  if (parsePrereleaseTag(kept) === null) return null;
  const bare = kept.slice(1);
...
function continuesThisRun(run: UpdateRun | null, lead: string, runId: string): boolean {
  ...
  return reached !== null && compareRelease(lead, reached) > 0;
}
