...
import { compareRelease } from "./update.ts";
import { inFlight, UPDATE_RUN_SCHEMA, type UpdateRun, type UpdateRunState } from "./update-run.ts";
...
/**
 * The accepted start sent in the 202, before the detached writer publishes its first record.
 * Capture `at` BEFORE launching so later records supersede this initial reading. No pid is
 * invented, and nothing is written to update.json: the detached updater remains its only writer.
 */
export function acceptedUpdateRun(a: { from: string; to: string; runId: string; at: number }): Omit<UpdateRun, "pid"> {
  return {
...
export function updateStartVerdict(req: UpdateStartRequest, state: UpdateStartState): UpdateStartVerdict {
  ...
  if (would === null || compareRelease(would, state.current) <= 0) {
    return refuse(409, "update.none_available");
  }
...
export function memberBehind(member: LevelMember, current: string): boolean {
  ...
  return compareRelease(member.version, current) < 0;
}
...
export function legStillFailed(leg: LevelLeg, crew: readonly LevelMember[], current: string): boolean {
  ...
  return compareRelease(version, current) < 0;
}
