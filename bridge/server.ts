...
export function startServer(opts: {
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      const browserGate = (level: "read" | "write"): Response | null =>
        guard(req, cfg, level, pairing, peerAddress);
      ...
      if (pathname === "/api/update/check" && req.method === "GET") {
        ...
        const freshSweep = opts.crewLead?.request({ freshPreflight: true });
      ...
      if (pathname === "/api/update" && req.method === "POST") {
        ...
        try {
          ...
          const run = acceptedUpdateRun({ from: status.current, to: verdict.to, runId, at: Date.now() });
          ...
          return json(
            { ok: true, to: verdict.to, major: verdict.major, run },
            req.headers.get("accept-encoding"),
            202,
          );
        } finally {
