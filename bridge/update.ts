...
export function updatesNewerThan(tags: string[], current: string): string[] {
  // The install's own release FAMILY, as everywhere else: a `+ys.N` install is told about `+ys`
  // releases and a bare install about bare ones (releaseFamily), and "newer" is compareRelease so
  // on this fork `1.1.0+ys.2` is news to a `1.1.0+ys.1` install.
  const family = releaseFamily(tags, current);
  const major = majorOf(current);
  const train = major !== null && followsTrain(current, latestReleaseInMajor(family, major));
  ...
  for (const tag of family) {
    ...
    const v = versionOfTag(parsed);
    if (compareRelease(v, current) <= 0) continue;
    seen.add(v);
  }
  return [...seen].toSorted(compareRelease);
}
...
export function shouldNotify(a: {
  ...
  // "Newer" is compareRelease: on this fork `1.1.0+ys.2` is news to a `1.1.0+ys.1` install.
  if (compareRelease(a.latest, a.current) <= 0) return { send: false };
  ...
  const unannouncedUrgent =
    a.urgent !== undefined &&
    (a.lastNotified === null || compareRelease(a.lastNotified, a.urgent.version) < 0);
  ...
  const folded =
    announced === null ? candidates : candidates.filter((v) => compareRelease(v, announced) > 0);
...
export function newestUrgent(
  ...
  for (const { version, reading } of readings) {
    ...
    if (found !== null && compareRelease(version, found.version) <= 0) continue;
