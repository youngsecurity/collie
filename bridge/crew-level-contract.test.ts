...
import { compareRelease, compareSemver } from "./update.ts";
import { crewNeedsLevelling, legStillFailed, memberBehind, type CrewUpdateRow } from "./update-action.ts";
import { compareRelease as phoneCompareRelease, compareSemver as phoneCompareSemver } from "../web/src/lib/semver.ts";
import {
...
describe("the version order is the same on both sides", () => {
  const PAIRS: readonly [string, string][] = [
    ...
    ["1.0.0+ab12cd3", "1.0.0"],
    ["unknown", "1.0.0"],
    ["1.11.0+ys.1", "1.11.0+ys.2"],
    ["1.11.0+ys.2", "1.11.0+ys.10"],
    ["1.11.0+ys.2.ab12cd3", "1.11.0+ys.2"],
    ["1.11.0+ys.2", "1.11.0"],
    ["1.11.0-rc.1+ys.2", "1.11.0+ys.1"],
  ];
  for (const [a, b] of PAIRS) {
    test(`${a} vs ${b}`, () => {
      expect(phoneCompareSemver(a, b)).toBe(compareSemver(a, b));
      expect(phoneCompareSemver(b, a)).toBe(compareSemver(b, a));
      expect(phoneCompareRelease(a, b)).toBe(compareRelease(a, b));
      expect(phoneCompareRelease(b, a)).toBe(compareRelease(b, a));
    });
  }
});

describe("fork crew release ordering", () => {
  const current = "1.11.0+ys.2";
  for (const [version, behind] of [
    ["1.11.0+ys.1", true],
    ["1.11.0+ys.2", false],
    ["1.11.0+ys.10", false],
    ["1.11.0+ys.2.ab12cd3", false],
  ] as const) {
    test(`${version} relative to ${current}`, () => {
      const row = member("peer", version);
      const leg = { name: "peer", state: "rolled-back" };
      expect(compareRelease(version, current) < 0).toBe(behind);
      for (const readsBehind of [memberBehind, phoneMemberBehind]) {
        expect(readsBehind(row, current)).toBe(behind);
        expect(readsBehind({ ...row, installKind: "packaged" }, current)).toBe(false);
      }
      for (const readsFailure of [legStillFailed, phoneLegStillFailed]) {
        expect(readsFailure(leg, [row], current)).toBe(behind);
      }
      for (const needsRun of [crewNeedsLevelling, phoneCrewNeedsLevelling]) {
        expect(needsRun([row], [], current)).toBe(behind);
        expect(needsRun([row], [leg], current)).toBe(behind);
      }
    });
  }
});
