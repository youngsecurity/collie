...
describe("compareRelease / releaseFamily: the fork family (decision D3)", () => {
  ...
  it("shouldNotify reads the counter: +ys.2 is news to a +ys.1 install", () => {
    const at = { lastPushedAt: null, now: new Date("2026-01-01T10:00:00") };
    expect(shouldNotify({ current: "1.1.0+ys.1", latest: "1.1.0+ys.2", newerVersions: ["1.1.0+ys.2"], lastNotified: null, ...at }))
      .toEqual({ send: true, versions: ["1.1.0+ys.2"] });
    expect(shouldNotify({ current: "1.1.0+ys.2", latest: "1.1.0+ys.2", newerVersions: [], lastNotified: null, ...at }))
      .toEqual({ send: false });
    expect(shouldNotify({ current: "1.1.0+ys.1", latest: "1.1.0+ys.2", newerVersions: ["1.1.0+ys.2"], lastNotified: "1.1.0+ys.2", ...at }))
      .toEqual({ send: false });
  });

  it("updatesNewerThan stays on the family and orders the counter", () => {
    const tags = ["v1.1.0", "v1.1.0+ys.1", "v1.1.0+ys.2", "v1.2.0", "v1.2.0+ys.1", "v2.0.0+ys.1"];
    // A +ys.1 install: the bare 1.1.0 and 1.2.0 are invisible, the fork's own newer releases are listed
    // oldest first, and the next major stays out of a routine digest.
    expect(updatesNewerThan(tags, "1.1.0+ys.1")).toEqual(["1.1.0+ys.2", "1.2.0+ys.1"]);
    // A bare install: the fork's tags are invisible.
    expect(updatesNewerThan(tags, "1.1.0")).toEqual(["1.2.0"]);
  });
});
...
describe("shouldNotify", () => {
  ...
  it("an urgent fork counter interrupts once and still respects the morning floor", () => {
    const update = {
      current: "1.9.1+ys.1",
      latest: "1.9.1+ys.3",
      newerVersions: ["1.9.1+ys.2", "1.9.1+ys.3"],
      lastNotified: "1.9.1+ys.2",
      lastPushedAt: hoursAgo(AT_10AM, 1),
      urgent: { version: "1.9.1+ys.3", reason: "Updating leaves the service stopped." },
    };
    expect(gate(update)).toEqual({ send: true, versions: ["1.9.1+ys.3"] });
    expect(gate({ ...update, now: new Date(2026, 0, 15, 3, 0, 0) })).toEqual({ send: false });
    expect(gate({ ...update, lastNotified: "1.9.1+ys.3" })).toEqual({ send: false });
    expect(gate({ ...update, latest: "1.9.1+ys.4", newerVersions: ["1.9.1+ys.4"],
      lastNotified: "1.9.1+ys.3" })).toEqual({ send: false });
  });
...
describe("the release reading", () => {
  ...
  it("newestUrgent picks the NEWEST marked release in the delta, or null", () => {
    ...
    // Fork counters order urgency too, numerically and independently of input order.
    for (const versions of [["1.9.1+ys.2", "1.9.1+ys.10"], ["1.9.1+ys.10", "1.9.1+ys.2"]]) {
      expect(newestUrgent(versions.map((version) => at(version, version)))).toEqual({
        version: "1.9.1+ys.10",
        reason: "1.9.1+ys.10",
      });
    }
