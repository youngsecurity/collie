...
import { bareVersion, crossesMajor } from "@/lib/semver";
...
export function useUpdateScreen(): UpdateScreen {
  ...
  const unmatched = claim !== null && !claim.peersOnly && claim.runId !== null && run !== undefined && run.runId !== claim.runId;
  const orphanClaim = claim !== null && answered && (view.phase === "none" || unmatched) ? claim : null;
  useEffect(() => {
    if (orphanClaim === null) return;
    const timer = setTimeout(() => {
      if (getUpdateClaim() !== orphanClaim) return;
      clearUpdateStarted();
      // Absence is not proof the updater stopped. Release the local claim, not the server's lock,
      // and leave a persistent notice rather than inventing a terminal run record.
      if (!orphanClaim.peersOnly) setStatus(t("updateScreen.startUnreported"), "warn", null);
    }, CLAIM_GRACE_MS);
  ...
  const tryAgain = useCallback(() => {
    ...
    clearUpdateStarted();
    if (target === null) return;
    const current = bareVersion(from ?? "");
    const version = bareVersion(target);
    const major = crossesMajor(current, version);
    openUpdateMode({ kind: major ? "major" : hasMembers ? "crew" : "single", version, major, peersOnly: false, current });
