...
import { __resetUpdateAsk, getUpdateAsk } from "@/lib/update-ask";
...
describe("rolled back card — the machine is named, the log is there, and there is a way on", () => {
  ...
  it.each(["2.0.0+ys.1", "v2.0.0+ys.1"])("a failed major crossing to %s asks for major consent again", async (to) => {
    renderCard(info({ current: "1.13.1+ys.1", run: runAt("rolled-back", { from: "v1.13.1+ys.1", to }) }));
    await userEvent.click(await screen.findByRole("button", { name: "Retry" }));
    expect(getUpdateAsk()).toMatchObject({ kind: "major", major: true, version: "2.0.0+ys.1", peersOnly: false });
  });
