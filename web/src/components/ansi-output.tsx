...
export interface AnsiOutputProps {
   ...
   *  their grid (upstream defaults to true for prose, and since 1.4 a table pans in its own scroller
   *  inside that wrap, lib/table-run.ts). Enable Wrap lines in the Display sheet for prose. */
  wrap?: boolean;
...
export const AnsiOutput = memo(function AnsiOutput({
  ...
  const paint =
    colors !== undefined && (colors.foreground !== "" || colors.background !== "")
      ? colors
      : undefined;
  const surface = paint !== undefined ? "painted" : rendersNativeMirror(agent, nativeMirror) ? "native" : "inverted";
      ...
      return (
        <span
          ...
          className={cn(
            ...
            //
            // Native and custom-painted surfaces have no outer filter to cancel, so the
