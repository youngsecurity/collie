...
describe("segmentStyle", () => {
  ...
  it.each(["", "#00ff00"])("preserves explicit muted ANSI over foreground %s", (foreground) => {
    const style = segmentStyle(seg({
      muted: true,
      fg: "rgb(255,165,216)",
      style: { color: "rgb(255,165,216)", fontWeight: 700, opacity: 0.5 },
    }), foreground);
    expect(style).toMatchObject({ color: "rgb(255,165,216)", fontWeight: 400, opacity: 1 });
  });

  it("composes custom muted foreground with mobile transparency", () => {
    const style = segmentStyle(seg({
      muted: true,
    ...
    expect(style).toEqual({
      color: "#00ff00", fontWeight: 400, opacity: 1,
      "--terminal-seg-bg": "rgb(240,240,240)",
    });
  });

  it("composes both adapter hints without dropping the emitted foreground or fill", () => {
    const style = segmentStyle(seg({
      fg: "rgb(250,250,249)",
      lightDarkFg: true,
      mobileTransparentBg: true,
      style: { color: "rgb(250,250,249)", backgroundColor: "rgb(240,240,240)" },
    }), "#00ff00");
    expect(style).toEqual({
      color: "var(--terminal-light-dark-fg, rgb(250,250,249))",
      "--terminal-seg-bg": "rgb(240,240,240)",
    });
  });
