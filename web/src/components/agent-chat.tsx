...
export function AgentChat({
  ...
  return (
    ...
    <CompactStripLabels>
      {/* Use the available browser width, not a desktop column cap. Keep the viewport bound,
          flex shrink guard and horizontal containment: a long terminal line must pan inside its
          mirror rather than widen the page. This does not resize the underlying terminal. */}
      <div className="mx-auto flex min-h-0 w-full min-w-0 max-w-[100dvw] flex-1 flex-col overflow-x-hidden">
        {/* Header — this route's contribution to the ONE header shell, which is mounted above the
            ...
            `width="full"` keeps the header aligned with the mirror, strips and composer on every
            viewport. The history route uses the same width when navigating to the transcript. */}
        <RouteHeader
        ...
        >
          ...
          {display && (
            <CardDock
              blocks={blocks}
              colors={mirrorFace.colors}
              nativeMirror={rendersNativeMirror(mirrorAgent, mirrorOverride)}
              ...
              <Collapse open={!composing && statusLines.length > 0}>
                ...
                <div
                  className={cn(
                    ...
                    // where the TUI drew it. Unpainted Muse keeps its native ground (.adr/0047).
                    // Operator colors take precedence and never invert, on either surface.
                    rendersNativeMirror(agent?.agent) && mirrorFace.colors === undefined ? MUSE_MIRROR : MIRROR_SPACE,
                    !rendersNativeMirror(agent?.agent) && mirrorFace.colors === undefined && MIRROR_INVERT,
                    mirrorFace.className,
                  )}
                  style={mirrorFace.style}
                >
                  {statusLines.map((row, i) => (
                    ...
                    <div key={i} className="truncate">
                      {row.segments.map((s, si) => (
                        ...
                        <span
                          key={si}
                          style={segmentStyle(s, mirrorFace.colors?.foreground)}
