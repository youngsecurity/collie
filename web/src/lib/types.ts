...
export interface UpdateRun {
  ...
  updatedAt: number;
  /**
   * The updater's pid. Absent from the standby answer and the accepted start (202), which
   * describe progress without a process handle; nothing on this side reads it.
   */
  pid?: number;
  attempt: number;
  ...
  reason?: string;
  /**
   * A bounded tail of what the run has to show for itself. On a failure it is the service log,
   * credential-scrubbed; while staging it is the fetch-and-build progress (M20/10). Never both, and
   * absent on a bridge older than either. Never from the standby door: that port is ungated, and a
   * log line is not a body a stranger may read.
   */
  logTail?: string;
  /**
...
export interface UpdatePeerLeg {
  name: string;
  /** OPEN, see the type: every reader needs a fallback branch, and `update-crew.ts`'s `legInFlight` and
