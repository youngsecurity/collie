...
import type { UpdateCrewMember, UpdatePeerLeg, UpdatePeerLegState, UpdateRun, UpdateRunState } from "./types";
import { peerStateWord } from "./update-crew";
...
describe("fork update compatibility", () => {
  it("keeps an unfamiliar peer state active with the card's fallback word", () => {
    // SAFETY: simulate a newer bridge's state outside this bundle's known union.
    const state = "levitating" as UpdatePeerLegState;
    const view = read({ run: run("done", { peers: [leg("workshop", state)] }), crew: [] });
    expect(view.rows.find((row) => row.name === "workshop")).toMatchObject({
      word: peerStateWord(state), status: "active", dim: false,
    });
    expect(view.phase).toBe("members");
  });

  it("does not turn a full fork-counter update into a crew-only run", () => {
    const view = read({
      claim: null,
      crewRun: { legs: WAITING, settledAt: null, to: "1.12.0+ys.2", current: "1.12.0+ys.1" },
    });
    expect(view.inFlight).toBeNull();
    expect(view.mode).toBe("hidden");
  });

  it("keeps an older fork-counter member queued until its release arrives", () => {
    const view = read({
      claim: { ...CLAIM, peersOnly: true, target: "1.12.0+ys.2" },
      crew: [{ name: "minibuch", version: "1.12.0+ys.1", verdict: "green", reasons: [], asOf: NOW }],
      crewRun: { legs: [], settledAt: null, to: "1.12.0+ys.2", current: "1.12.0+ys.2" },
    });
    expect(view.rows.find((row) => row.name === "minibuch")).toMatchObject({
      status: "queued", versions: "1.12.0+ys.1 → 1.12.0+ys.2",
    });
  });

  it("fetches the phone's newer fork counter instead of declaring the old bundle done", () => {
    const view = read({
      run: run("done", { to: "1.12.0+ys.2", peers: [], settledAt: NOW - 1_000 }),
      crew: [],
      claim: { ...CLAIM, target: "1.12.0+ys.2" },
      bundle: { id: "bundle-a", version: "1.12.0+ys.1" },
      serverStale: true,
    });
    expect(view.phase).toBe("phone");
    expect(view.kickPhone).toBe(true);
  });

  it("recognizes a dev stamp before fork metadata as the same release", () => {
    const view = read({
      run: run("done", { to: "1.12.0+ys.2", peers: [], settledAt: NOW - 1_000 }),
      crew: [],
      claim: { ...CLAIM, target: "1.12.0+ys.2" },
      bundle: { id: "bundle-a", version: "1.12.0-dev+ys.2" },
    });
    expect(view.phase).toBe("done");
    expect(view.rows.at(-1)).toMatchObject({ status: "ok", versions: "1.12.0+ys.2" });
  });
});
