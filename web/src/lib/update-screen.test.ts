...
it("keeps an unfamiliar peer state moving with the same fallback word as the card", () => {
  // SAFETY: deliberately plant a newer bridge's wire value outside this bundle's known union.
  const state = "levitating" as UpdatePeerLegState;
  const view = read({ run: run("staging", { peers: [{ name: "workshop", state }] }) });
  const peer = view.rows.find((row) => row.name === "workshop");
  expect(peer).toMatchObject({ word: peerStateWord(state), moving: true, settled: false });
  expect(peer?.word).toBe("in progress");
  expect(view.mode).not.toBe("hidden");
});
...
describe("a crew-only run: tapped here or elsewhere, in flight, failed, done, escaped", () => {
  ...
  it("keeps a full fork-counter update out of the crew-only screen", () => {
    const view = read({
      startedHere: true,
      run: run("done", { to: "1.11.0+ys.1", updatedAt: NOW - 3 * 86_400_000 }),
      crewRun: { legs: MOVING, settledAt: null, to: "1.11.0+ys.2", current: "1.11.0+ys.1" },
    });
    expect(view.inFlight).toBeNull();
    expect(view.rows.some((row) => row.word === "already up to date")).toBe(false);
  });

  it("keeps an older fork-counter member waiting until its release arrives", () => {
    const view = read({
      startedHere: true,
      crew: [{ name: "minibuch", version: "1.11.0+ys.1", verdict: "green", reasons: [], asOf: NOW }],
      crewRun: { legs: [], settledAt: null, to: "1.11.0+ys.2", current: "1.11.0+ys.2" },
    });
    expect(view.rows.find((row) => row.name === "minibuch")).toMatchObject({ word: "waiting", settled: false });
  });
