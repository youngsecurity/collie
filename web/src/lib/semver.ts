 ...
 *  Other build metadata has no precedence, including a hash appended after the counter. */
export function compareRelease(a: string, b: string): number {
  const bySemver = compareSemver(a, b);
  if (bySemver !== 0) return bySemver;
  const x = Number(FORK_COUNTER.exec(a.trim())?.[1] ?? 0);
  const y = Number(FORK_COUNTER.exec(b.trim())?.[1] ?? 0);
