...
function bundleAtLeast(version: string, target: string): boolean {
  ...
  try {
    return compareRelease(bare, target) >= 0;
  } catch {
    return false;
  }
}
...
function leadView(input: UpdateScreenInput, run: UpdateRun): UpdateScreenView {
  ...
  // A named claim needs a matching ID, including when an older record has no ID at all.
  // Claims from older bridges may themselves lack an ID; keep their compatibility reading.
  const mine =
    input.startedHere &&
    !(claim !== null && claim.peersOnly) &&
    !(claim?.runId != null && run.runId !== claim.runId);
