...
import { compareRelease } from "./semver";
...
function legWord(state: UpdatePeerLegState, version: string | null): string {
  switch (state) {
    ...
    case "idle":
      return t("updateScreen.state.idle");
    default:
      // A newer bridge can name a state this bundle has never seen. Like the update card, keep
      // its row moving and give it a word rather than silently showing an empty label.
...
function peerRowsOf(
  ...
  for (const member of input.crew) {
    ...
    const level = levelTo !== undefined && member.version !== null && compareRelease(member.version, levelTo) >= 0;
    rows.push({
      name: member.name,
      lead: false,
...
function crewOnlyRun(crew: UpdateScreenCrewRun | null): UpdateScreenCrewRun | null {
  ...
  return compareRelease(crew.to, crew.current) === 0 ? crew : null;
}
