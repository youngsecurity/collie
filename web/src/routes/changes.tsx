...
export function ChangesRoute() {
  ...
  const backToList = () => {
    // SAFETY: openFile writes FromList; cold-entry seeding writes `from` instead, so the
    // fromList marker can be absent. Both forms must step onto their existing parent entry.
    const fromList = (location.state as Partial<FromList> | null)?.fromList === true;
    if (fromList && canStepBack()) navigate(-1);
    else nav.upTo(commitRepo !== null ? commitPathTo(commitRepo) : pathTo());
  };
