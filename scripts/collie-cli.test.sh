...
# These sandboxes make real commits and tags, so isolate them from user-wide hooks and signing: a
# global tag.gpgSign would turn `git tag` into an interactive signing operation and hang this
# unattended suite, and a user hooksPath could run hooks that expect a real checkout.
git_q() {
  git -c init.defaultBranch=main -c user.email=t@t -c user.name=t \
...
if run_stripped HOME="${TMP_ROOT}/update-home" HERDR_PLUGIN_CONFIG_DIR="${TMP_ROOT}/update-config" \
  PATH="${U_BIN}:${BASE_PATH}" COLLIE_MUX=herdr COLLIE_PORT="$U_PORT" COLLIE_PLUGIN_ROOT="$MANAGED" \
