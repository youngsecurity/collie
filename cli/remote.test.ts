...
describe("commitlessLeadLines (#248)", () => {
  ...
  test("the source-only fork names its source release instead of nonexistent binary payloads", () => {
    for (const kind of [{ kind: "binary" }, { kind: "packaged" }] as const) {
      for (const version of ["1.11.0+ys.2", "1.11.0-rc.1+ys.1", "unknown"]) {
        const lines = at(kind, { verb: "add", host: "nas" }, { repo: "youngsecurity/collie", version })!.join("\n");
        ...
        expect(lines).toContain("collie crew invite");
        expect(lines).not.toContain("install.sh");
      }
    }
    const update = at({ kind: "binary" }, { verb: "update" }, {
      repo: "youngsecurity/collie", version: "1.11.0+ys.2",
    })!.join("\n");
    expect(update).toContain("from source");
    expect(update).not.toContain("install.sh");
    expect(update).not.toContain("collie update --to-tag");
  });
