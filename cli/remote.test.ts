...
function releaseHarness(opts: HarnessOptions = {}): Harness {
  return harness({
    ...
    ...opts,
    env: { COLLIE_UPDATE_REPO: "AltanS/collie", ...opts.env },
    extraFiles: {
...
describe("source-only crew releases", () => {
  test("releaseOf retains fork counters while removing build stamps and annotations", () => {
    for (const [input, expected] of [
      ["1.11.0+ys.2.abc123d (manifest; web not built)", "1.11.0+ys.2"],
      ["1.11.0-rc.1+ys.1.abc123d", "1.11.0-rc.1+ys.1"],
      ["1.11.0+abc123d", "1.11.0"],
    ]) expect(releaseOf(input!)).toBe(expected!);
  });

  test("the fork names source instructions for both commitless kinds and verbs", () => {
    for (const kind of [{ kind: "binary" }, { kind: "packaged" }] as const) {
      for (const version of ["1.11.0+ys.2", "1.11.0-rc.1+ys.1", "unknown"]) {
        for (const asking of [{ verb: "update" }, { verb: "add", host: "nas" }] as const) {
          const lines = commitlessLeadLines(kind, { root: ROOT, repo: "youngsecurity/collie", version }, asking)!.join("\n");
          expect(lines).toContain("from source");
          expect(lines).toContain("https://github.com/youngsecurity/collie/blob/main/docs/install.md#this-fork");
          expect(lines).toContain(version === "unknown" ? "`collie version` names it" : `v${version}`);
          expect(lines).not.toContain("vunknown");
          expect(lines).not.toContain("install.sh");
          expect(lines).not.toContain("collie update --to-tag");
          if (asking.verb === "add") expect(lines).toContain("collie crew invite");
        }
      }
    }
  });

  test("a fork build cannot select upstream binary payloads through a repo override", async () => {
    for (const kind of [{ kind: "binary" }, { kind: "packaged" }] as const) {
      const h = releaseHarness({
        installKind: kind,
        extraFiles: { [`${ROOT}/web/dist/build-info.json`]: JSON.stringify({ version: "1.11.0+ys.2.abc123d" }) },
      });
      expect(await run(h)).toBe(EXIT.FAIL);
      expect(text(h.io)).toContain("from source at v1.11.0+ys.2");
      expect(h.calls).toEqual([]);
      expect(h.bundles).toBe(0);
    }
  });

  test("a packaged fork lead refuses before SSH even without a readable version", async () => {
    const h = harness({ installKind: { kind: "packaged" } });
    expect(await run(h)).toBe(EXIT.FAIL);
    expect(text(h.io)).toContain("from source at v<version>");
    expect(h.calls).toEqual([]);
  });
});
...
describe("the release route", () => {
  ...
  test("a lead whose own version cannot be read refuses before the first ssh byte", async () => {
    ...
    const h = harness({ installKind: { kind: "binary" }, env: { COLLIE_UPDATE_REPO: "AltanS/collie" } });
    expect(await run(h)).toBe(EXIT.FAIL);
