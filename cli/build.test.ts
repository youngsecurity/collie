...
function withGitBuildFixture<T>(
  ...
  try {
    ...
    copyFileSync(join(sourceRoot, "web", "vite-build.ts"), join(web, "vite-build.ts"));
  ...
  test("contains compiler cwd scratch before release build-info samples a clean checkout", () => {
    ...
    withGitBuildFixture(
      (fixture) => {
        ...
        const metadataSeparator = vitePackageVersion(root).includes("+") ? "." : "+";
        expect(info.id).toStartWith(`${info.version}${metadataSeparator}${info.sha}.`);
