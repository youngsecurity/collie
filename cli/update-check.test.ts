...
// This fork: `youngsecurity/collie` IS the default update source (cli/install-kind.ts), so the healthy
// origin here is the fork itself, and upstream's `AltanS/collie` would be the mismatch.
...
describe("preflight — the upstream check", () => {
  ...
  test("a read-only tag listing needs no credential: an ssh origin is listed over https, with a timeout", async () => {
    const h = harness({
      answers: [[`${GIT} remote get-url origin`, { stdout: "git@github.com:youngsecurity/collie.git\n" }]],
    });
    await preflight(h.deps);
  ...
  test("an origin that is not the configured update source is red and names both", async () => {
    ...
    expect(check.reason).toContain("github.com/youngsecurity/collie");
...
describe("preflight crew — the members of a lead", () => {
  ...
  test("a member that claims red with only malformed checks is still red, with one check saying so", async () => {
    ...
    expect((await preflight(honest.deps)).crew![0]!.checks.map((c) => c.id)).not.toContain("report");
  });
  ...
  test("fork release metadata does not bypass the protocol floor", () => {
    expect(skewCheck("1.7.0+ys.2", "1.9.0+ys.1").verdict).toBe("red");
    expect(skewCheck("1.7.0", "1.9.0+ys.1").verdict).toBe("red");
    expect(skewCheck("1.7.0+ys.2", "1.9.0").verdict).toBe("red");
    expect(skewCheck("1.8.0+ys.1", "1.9.0+ys.1").verdict).toBe("amber");
    expect(skewCheck("1.7.0+ys.2", "1.8.0+ys.2").verdict).toBe("amber");
    expect(skewCheck("1.9.0+ys.1", "1.9.0+ys.1").verdict).toBe("green");
    expect(skewCheck("1.7.0-dev+ys.2", "1.9.0+ys.1").verdict).toBe("amber");
    expect(skewCheck("1.7.0+ys.2", "1.9.0-dev+ys.1").verdict).toBe("amber");
    expect(skewCheck("1.7.0+ys.bad", "1.9.0+ys.1").verdict).toBe("amber");
  });

  test.each([
    ["1.9.0+ys.1", "1.7.0+ys.2", "red"],
    ["1.9.0", "1.7.0", "red"],
    ["1.9.0+ys.1", "1.7.0", "red"],
    ["1.9.0", "1.7.0+ys.2", "red"],
    ["1.9.0+ys.1", "1.8.0+ys.2", "amber"],
    ["1.8.0+ys.2", "1.7.0+ys.2", "amber"],
    ["1.9.0-dev+ys.1", "1.7.0+ys.2", "amber"],
    ["1.9.0+ys.1", "1.7.0-dev+ys.2", "amber"],
    ["1.9.0+ys.1", "1.7.0-rc.1+ys.2", "amber"],
  ] as const)("built lead %s reads built member %s as %s in the preflight walk", async (ours, theirs, verdict) => {
    const built = JSON.stringify({ version: ours, sha: "cafebabe" });
    const reported = bareVersionFrom(JSON.stringify({ version: theirs, sha: "deadbee" }), null);
    const h = harness({
      installed: ours,
      files: { [`${ROOT}/web/dist/build-info.json`]: built },
      store: lead(["nas"]),
      ops: { nas: record() },
      remote: () => (script) => script.includes("update --check")
        ? ok(JSON.stringify({ schema: 1, verdict: "green", checks: [] }))
        : ok(probeOut({ version: reported })),
    });
    const report = await preflight(h.deps);
    const skew = report.crew?.[0]?.checks.find((check) => check.id === "version");
    expect(skew?.verdict).toBe(verdict);
    expect(skew?.reason).toContain(reported);
    expect(skew?.reason).toContain(bareVersionFrom(built, null));
    expect(report.verdict).toBe(verdict);
    expect(await cmdUpdateCheck(h.deps, ["--json"])).toBe(verdict === "red" ? EXIT.FAIL : EXIT.OK);
  });
...
describe("parseReport validates every element", () => {
  ...
  test("crew rows are read the same way: a malformed member is dropped, its checks are filtered", () => {
    const doc = {
      schema: 1,
      verdict: "green",
      checks: [good],
    ...
    const report = parseReport(JSON.stringify(doc))!;
    expect(report.crew?.map((m) => `${m.memberId}:${m.verdict}:${m.checks.length}`)).toEqual(["bluefin:green:1", "workshop:red:1"]);
    // A `crew` that is not an array reads as no crew at all.
    expect(parseReport(JSON.stringify({ ...doc, crew: {} }))!.crew).toBeUndefined();
  });
});
