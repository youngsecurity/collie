...
export function parseReport(stdout: string): PreflightReport | null {
  ...
  if (!Array.isArray(members)) return report;
  const crew = members.map(asMember).filter((m): m is PreflightMember => m !== null);
  return { ...report, crew };
}
...
/**
 * Stable releases, including the SHA that collieVersionBare reads from the built bundle.
 * Upstream stamps `+<sha>`; the fork stamps `+ys.N.<sha>`. Neither changes protocol capability.
 * Prereleases and unrecognized metadata still cannot establish that a member is known-old.
 */
const RELEASE_VERSION = /^\d+\.\d+\.\d+(?:\+(?:ys\.\d+(?:\.[0-9a-f]{4,40})?|[0-9a-f]{4,40}))?$/i;
...
/** True for stable releases when this lead is at 1.9.0 or newer and the member is below the floor. */
