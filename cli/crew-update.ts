...
async function updateRun(deps: Wired, args: readonly string[]): Promise<number> {
  ...
  const refusal = commitlessLeadLines(kind, {
    root: deps.ctx.root,
    version: releaseOf(collieVersionBare(deps.ctx.root, (p) => deps.files.read(p))),
    repo: updateRepoOf(deps.ctx.env),
  }, { verb: "update" });
  if (refusal !== null) {
    for (const text of refusal) deps.io.err(text);
    return EXIT.FAIL;
  }
  ...
  try {
    ...
    if (behind && !(await updateLead(deps, version, build.commit))) {
