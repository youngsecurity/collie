...
export function planToTag(a: {
  ...
  // The family rule `planUpdate` applies, spelled as a refusal: a `+ys.N` install pinned onto a bare
  // upstream tag would discard this fork's hardening, and a bare install pinned onto a fork tag would
  // inherit a build it never chose. Which train an install is on is read off its version, never a flag.
  if (familyOnly([target], a.installed).length === 0) {
    return {
      kind: "refused",
  ...
  // That is the lead's leg of `crew update` on a checkout that advanced without being built (#24).
...
export function requireHerdrMinimum(deps: UpdateDeps): boolean {
  ...
  if (current === null || compareSemver(minimum, current) > 0) {
    ...
    deps.io.err(`       Upgrade Herdr, then rerun: ${herdrActionCommand("update", deps.ctx.instance)}`);
    return false;
  }
  return true;
}
...
function updateLinkedMajor(
  ...
  const fetch = isShallow(deps.exec, root)
    ...
    : ["fetch", "origin", spec];
  if (git(fetch) !== EXIT.OK) return { code: EXIT.FAIL, moved: false, to: null, higher: null };
  ...
  if (git(["merge", "--ff-only", "FETCH_HEAD"]) !== EXIT.OK) return { code: EXIT.FAIL, moved: false, to: null, higher: null };
  ...
  return { code: EXIT.OK, moved: headNow() !== before, to: plan.target.version, higher: null };
}
...
export async function cmdApplyUpdate(deps: UpdateDeps, args: readonly string[] = []): Promise<number> {
  // Both verbs below land NEW code on a Herdr install, so the manifest's floor is enforced before
  // either moves anything (this fork; see requireHerdrMinimum).
...
export function listVersions(
  ...
  return deps.files
    ...
    .toSorted((a, b) => compareRelease(a.version, b.version));
}
...
async function rollbackCheckout(deps: UpdateDeps, layout: BinaryLayout): Promise<number> {
  ...
  const older = kept.filter((v) => compareRelease(v.version, at.version) < 0);
  const target = older[older.length - 1];
  if (target === undefined) {
...
function handOff(
  ...
  // The lock is held from here to the launch. A write or a `which` that throws (ENOSPC, EACCES) must
  // not leave it behind with no run to show for it: that is the same ten-minute refusal the runner's
  // own finally exists to prevent (#21), one step earlier. Everything inside this block happens
  // BEFORE anything is started, so "nothing started" is true of every throw it catches.
  function notStarted<TError>(err: TError): number {
  ...
  if (plan.confirms === "manager") {
    ...
    //
    // DELIBERATELY OUTSIDE the "nothing started" catch above. `runLogged` spawns the client FIRST and
    // appends its output to the log AFTER, so a throw out of it (ENOSPC on that append) may come back
    // with the runner already accepted by the manager. Releasing the lock and folding the record to
    // `idle` there would tell the operator nothing happened while the swap proceeds, and a retry typed
    // on that advice would race the live runner. A throw here propagates instead: the lock stays held
    // under this pid for the runner to inherit, the record stays `staging`, and if no runner ever
    // took it the staleness rule frees both, which is the ten-minute cost #21 accepts for the one
    // throw it cannot tell apart from a launch.
    ...
    // Back INSIDE a "nothing started" catch, and correctly so: `spawnDetached` opens the log before
    // it spawns, so a throw out of it is a throw from before the runner existed (#21).
    let pid: number | null;
    try {
