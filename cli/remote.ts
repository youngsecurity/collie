...
export function commitlessLeadLines(kind: InstallKind, lead: CommitlessLead, asking: CommitlessVerb): string[] | null {
  ...
  // The downloaded installer belongs to upstream, regardless of this CLI's default repository.
  const fork = lead.repo === "AltanS/collie" ? "" : `COLLIE_UPDATE_REPO=${lead.repo} `;
  const tag = parsed === null ? "v<version>" : `v${lead.version}`;
  const sourceOnly = lead.repo.toLowerCase() === "youngsecurity/collie";
  const install = sourceOnly
    ? [
    ...
    : [`         curl -fsSL https://colliepwa.dev/install.sh | ${fork}COLLIE_TAG=${tag} sh`];
