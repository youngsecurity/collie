...
import { forkCounterOf, parsePrereleaseTag } from "../bridge/update.ts";
import {
  CANDIDATE_QUESTION,
  markCandidates,
  mergeCandidates,
...
async function crewAddRun(deps: Wired, args: readonly string[]): Promise<number> {
  ...
  const refusal = commitlessLeadLines(kind, {
    root: deps.ctx.root,
    version: leadVersion,
    repo: updateRepoOf(deps.ctx.env),
  }, { verb: "add", host });
  if (refusal !== null) {
    for (const line of refusal) deps.io.err(line);
    return EXIT.FAIL;
  }
...
/**
 * A version string reduced to its release, preserving the fork counter but not the build stamp.
 *
...
export function releaseOf(version: string): string {
  const bare = version.trim().split(/\s+/)[0] ?? "";
  const core = bare.split("+")[0] ?? "";
  const fork = forkCounterOf(bare);
  return fork === null ? core : `${core}+ys.${fork}`;
}
...
/**
 * Source-only fork installs cannot use the upstream commitless release route (#248).
 * Checkout leads still push a bundle. Other release families retain the embedded installer path.
 */
export function commitlessLeadLines(kind: InstallKind, lead: CommitlessLead, asking: CommitlessVerb): string[] | null {
  ...
  if (lead.repo.toLowerCase() !== "youngsecurity/collie" && forkCounterOf(lead.version) === null) return null;
  ...
  const install = [
    `         Install youngsecurity/collie from source at ${tag}; this fork publishes no binary payloads.`,
    "         https://github.com/youngsecurity/collie/blob/main/docs/install.md#this-fork",
  ];
  const head = `error: ${lead.root} is a ${kind.kind} install, so it has no commit to push.`;
  if (asking.verb === "update") {
    return [head, `       On each member, install ${itsRelease} by hand:`, ...install];
  }
  const { host } = asking;
  return [
    head,
    `       Add ${host} by hand instead. On ${host}, install ${itsRelease}:`,
    ...install,
