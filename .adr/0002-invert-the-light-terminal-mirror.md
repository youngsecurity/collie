...
The per-pane override applies when no custom terminal colors are set. Custom foreground or
background colors remain absolute, including when a pane requests inversion. Clearing those
colors restores the pane's override or its agent default.
