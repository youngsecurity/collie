...
Stable `X.Y.Z+ys.N` releases are readable on both sides of the floor check, including the built
bundle's Git SHA: `X.Y.Z+ys.N.<sha>` on this fork and `X.Y.Z+<sha>` upstream. Those are the values
`collieVersionBare` supplies to the real preflight walk. Their upstream base sets protocol
capability; neither the fork counter nor the SHA changes it. Prerelease and development versions
still take the amber arm, including `1.9.0-dev+ys.1.<sha>`. Without this distinction, built leads
would skip the floor and admit an update roll with a known protocol-1 member.
