...
- **No binary payloads and no packages.** Fork releases are tagged `vX.Y.Z+ys.N` and ship
  source-only, so the `colliepwa.dev/install.sh` script, a binary `collie update`, and every route
  under [From a package](#from-a-package) (the Arch `collie-bin` recipe, the Nix flake, mise)
  install or update **upstream** Collie, not this fork. Use [the same result, from source](#the-same-result-from-source) or
