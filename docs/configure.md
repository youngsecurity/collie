...
you pan horizontally, so full-screen TUIs and box drawing keep their grid. Upstream Collie wraps by
default, and since 1.4 a table pans in its own scroller inside that wrap, so wrapping costs a table
nothing there either. Toggle **Wrap lines** in the composer's Display sheet (the gear beside Keys) to wrap prose
