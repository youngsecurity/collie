...
- **Default defensive controls.** Collie binds strictly to loopback interfaces, routes traffic
  ...
  On this fork host-header validation is peer-aware: a remote request's `Host` must match the
  discovered tailnet name or a `COLLIE_PUBLIC_HOSTS` entry, a loopback name is accepted only from a
  direct loopback socket peer, and `COLLIE_ALLOWED_ORIGINS` never expands that allowlist.
